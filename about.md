...
<h2 class="page-title pt-4">Blog Posts on Entrepreneurial Journey</h2>
<div class="mb-5">
{% for link in site.data.journey %}
    <h5 class="border-bottom row py-3 mb-1"><a href="{{ link.url }}" target="_blank" class="text-dark  text-decoration-none">{{ link.text }}</a></h5>
{% endfor %}
</div>
<h3 class="pt-4" id="expertise">My Expertise</h3>
Hands-on experience across Technology, Design, Sales, Marketing, and Operations.
{% for skill in site.data.skills %}
<div class="row py-2 border-bottom"><small class="col-md-3">{{ skill.category }}</small><div class="col-md-9"> {% for item in skill.items %}<small class="tag1 ">{{ item }}, </small>{% endfor %}</div></div>
{% endfor %}
